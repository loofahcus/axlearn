...
- Integrated: Type checking is part of the Bazel build process
- Dependency-aware: Pytype can find imported modules via Bazel's dependency resolution
- Generic: Works with any package structure (not hardcoded paths)
- Ajax-compatible: Uses same pytype flags as Ajax project (`-k -v 1`)
- No config file: Follows Ajax's approach of not using `pytype.cfg`
- Python 3.12 compatible: Upgraded for better pytype support
...
When Ajax gets Bazelize'd, this pytype integration will be available in Ajax since Ajax depends on AXLearn:
